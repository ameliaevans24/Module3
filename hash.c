...
void* hsearch(hashtable_t *htp, bool (*searchfn)(void* elementp, const void* se\
archkeyp), const char *key, int32_t keylen) {                                   
  uint32_t hashCode = SuperFastHash(elementp, keylen, htp -> size);             
  //not sure if im filliing in the qsearch properly                             
  void* found = qsearch(htp -> table[hashCode] -> qp, fn, searchkeyp);          
  return found;                                                                 
                                                                                
}                                                                               
                                                                                
void* hremove(hashtable_t *htp, bool (*searchfn)(void* elementp, const void* se\
archkeyp), const char *key, int32_t keylen) {  
  uint32_t hashCode = SuperFastHash(elementp, keylen, htp->size);               
                                                                                
  //not sure if i filled in the qremove properly, especially entering the function argument                                                                    
  void* removed = qremove(htp-> table[hashCode] -> qp, searchfn, searchkeyp);   
  return removed;                                                               
} 
